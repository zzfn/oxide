...
- [ ] 钩子触发机制
  ...
  - 工具调用前后钩子（`pre-tool-call`, `post-tool-call`）
  - 会话生命周期钩子（`session-start`, `session-end`）
- [ ] Shell 命令执行和输出捕获
- [ ] 钩子结果反馈和阻塞机制
  - 钩子失败时阻止操作继续
  - 钩子输出注入到 AI 上下文
...
- [x] 权限请求系统（声明）
- [x] 工具执行权限控制（三层防护架构）
  - [x] 配置级权限（allow/deny 列表）
    - `permissions.allow` 为空：默认允许所有（黑名单模式）
    - `permissions.allow` 非空：仅允许列表内工具（白名单模式）
    - `permissions.deny` 优先级最高，直接拒绝
  - [x] 运行时确认（危险工具交互式授权）
    - 危险工具：Edit, Write, Bash
    - 确认选项：允许本次/始终允许（会话）/始终允许（持久）/拒绝
  - [x] 工具包装器（执行前拦截）
    - 步骤1：检查配置禁止
    - 步骤2：检查是否需要用户确认
    - 步骤3：执行工具
...
- **Hooks 系统**: 事件驱动的 shell 命令执行
- **自动摘要**: 无限上下文支持
- **System Reminders**: 自动上下文提醒
- **环境上下文注入**: Git 状态、系统信息自动注入
- **代码引用跳转**: `file:line` 格式的可点击引用
- **WebSearch 工具**: 网页搜索功能
- **Notebook 工具**: Jupyter notebook 支持
- **AI 行为策略**: 工具使用策略、交互准则
...
#### Oxide 的三层防护架构
1. **配置级权限**（静态白名单/黑名单）
   - 位置：`~/.oxide/config.toml`，支持项目级覆盖
   - 白名单模式：`permissions.allow` 非空时仅允许列表内工具
   - 黑名单模式：`permissions.allow` 为空时默认允许所有
   - 适用场景：自动化/CI 环境，生产部署

2. **运行时确认**（危险工具交互式授权）
   - 危险工具：Edit, Write, Bash
   - 用户选择：允许本次/始终允许（会话）/始终允许（持久）/拒绝
   - 会话级缓存：避免重复询问

3. **工具包装器**（执行前拦截）
   - 每个工具调用前自动检查权限
   - 权限错误返回给 AI，由 AI 决定后续行为

#### Claude Code 的权限机制
- **Hooks 系统**：通过 shell 命令在工具调用时执行拦截
- **无静态白名单**：工具默认可用，依赖 hooks 动态控制
- **交互式设计**：更适合开发者交互式使用
