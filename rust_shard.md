...
#### D. 接入 Langfuse (Observability)

- **全链路追踪：** 基于 `opentelemetry` 和 `tracing` 库集成 Langfuse，记录每一个 Prompt 的输入输出、Latency 和 Token 消耗。
- **按需开启：** 仅在 Debug 模式且配置了环境变量（`LANGFUSE_PUBLIC_KEY` 等）时自动启用。
- **深度调试：** 利用 `tracing-opentelemetry` 将 Rust 的异步 Span 自动映射为 Langfuse 的 Trace，可视化分析 Tool 执行耗时和 LLM 决策链路。

### 6. 挑战与解决方案 (Challenges)

在复刻过程中，最复杂的不是调用 API，而是如何构建一个稳定、好用的 CLI 交互系统。

- **流式渲染与状态管理**
  - **挑战**：LLM 的 token 是流式输出的，同时 Tool 执行也会产生日志，两者混杂会导致终端界面闪烁或错乱。
  - **解法**：设计了一个 `StreamState` 状态机，严格管理 `Text` (文本流)、`Reasoning` (思考过程) 和 `ToolStatus` (工具状态) 的渲染优先级。利用 `crossterm` 进行光标回退和局部重绘，实现丝滑的"打字机+进度条"效果。

- **权限控制与安全性**
  - **挑战**：Agent 能力越强，潜在风险越大（如误删文件）。
  - **解法**：实现了三层防御体系：
    1. **静态配置**：`config.toml` 白名单/黑名单。
    2. **运行时拦截**：`PermissionManager` 劫持所有 Tool Call，危险操作（如 `Bash`, `Write`）强制弹出交互式确认框。
    3. **会话记忆**：支持"本次会话允许"，平衡安全性与易用性。

- **Prompt 的动态构建**
  - **挑战**：System Prompt 不是静态字符串，需要根据操作系统、当前目录 Git 状态、用户自定义规则动态变化。
  - **解法**：采用 `Builder` 模式实现的 `PromptBuilder`，像搭积木一样组装 `Core`、`Security`、`Context` 等模块，并动态注入 `RuntimeContext`（OS 版本、CWD 信息），确保 LLM 始终拥有正确的上下文。

- **异步任务编排**
  - **挑战**：既要监听 stdin 用户输入（由 `reedline` 阻塞管理），又要处理后台 Tool 执行和 LLM 流式响应。
  - **解法**：基于 `tokio` 的 Actor 模式思想，将 UI 渲染、LLM 交互、工具执行拆分为独立的异步任务，通过 Channel 通信，避免主线程阻塞。
